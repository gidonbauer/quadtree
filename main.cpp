...
auto main() noexcept -> int {
  ...
  Box<Float> bb{.x = 0.0, .y = 0.0, .w = 100.0, .h = 150.0};
  Quadtree<size_t, Float, 100UL> qt(bb);
  ...
  std::generate(std::begin(positions), std::end(positions), [&] {
    return Point{.x = dist_x(gen), .y = dist_y(gen)};
  });
  ...
  try {
    Box box{.x = static_cast<Float>(15.0),
    ...
    const auto naive_count = std::count_if(
        std::cbegin(ps), std::cend(ps), [&](const Point<Float>& p) { return box.contains(p); });
  ...
  try {
    ...
    t_begin                = std::chrono::high_resolution_clock::now();
    const auto naive_count = std::count_if(
        std::cbegin(ps), std::cend(ps), [&](const Point<Float>& p) { return circle.contains(p); });
    assert(naive_count >= 0);
    const auto dur_naive_find =
        std::chrono::duration<double>(std::chrono::high_resolution_clock::now() - t_begin);
